    ...
    var Utilidades = (function () {
        ...
        **/
        ...
        Utilidades.contiene = function (lista, elemento) {
            ...
            if (lista.length > 0) {
                for (var i = 0; i < lista.length; i++) {
                    if (lista[i] instanceof app.Escenario) {
                        if (!cvUtil.Utilidades.comprobarObjetoNulo(lista[i].estado))
                            lista[i].estado2 = lista[i].getCadenaEstado();
                        if (!cvUtil.Utilidades.comprobarObjetoNulo(lista[i].selloEditorial))
                            lista[i].sello2 = lista[i].getCadenaSello();
                        if (!cvUtil.Utilidades.comprobarObjetoNulo(lista[i].canalPropietario))
                            lista[i].propietario2 = lista[i].canalPropietario.getCadenaCompleta();
                    }
                    if (lista[i] instanceof app.Presupuesto) {
                        if (!cvUtil.Utilidades.comprobarObjetoNulo(lista[i].getEstadoPresupuesto()))
                            lista[i].estado2 = lista[i].getEstadoPresupuesto();
                        if (!cvUtil.Utilidades.comprobarObjetoNulo(lista[i].propietario))
                            lista[i].titulo = lista[i].getCadenaCompletaPropietario();
                    }
