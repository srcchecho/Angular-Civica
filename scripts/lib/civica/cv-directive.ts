...
*/
    export class CVFormDirective implements ng.IDirective {

        public restrict: string = "A";
        public priority: number = 2000;
        private $timeout: ng.ITimeoutService;
        /*public scope: Object = {
            
        };*/

        public link: (scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes, ctrl: ng.IFormController) => void;
         
        // @ngInject
        constructor(private $location: ng.ILocationService, private $rootScope: ng.IScope, private $compile: ng.ICompileService, $timeout: ng.ITimeoutService, $window: ng.IWindowService) {
            
            // It's important to add `link` to the prototype or you will end up with state issues.
            // See http://blog.aaronholmes.net/writing-angularjs-directives-as-typescript-classes/#comment-2111298002 for more information.
            CVFormDirective.prototype.link = (scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes, ctrl: ng.IFormController) => {

                //console.log("CVFormDirective.prototype.link....Atributes: " + attrs.cv-form);
                angular.element(document).ready(() => {
                    var selects: ng.IAugmentedJQuery = element.find('md-select');
                ...
                var necesitaCompilar: boolean = false;

                var elFormulario: HTMLFormElement = <HTMLFormElement>element[0];
                //alert('E=' + element);
                var objFormulario: Object = scope.$eval(elFormulario.name);
            	
                // Relaciona el nombre del input con el label de su campo
                var labelsCampos: Object = new Object();
                // Recupera el tipo de cada uno de los campos
                var tiposCampos: Object = new Object();
                ...
                var inputs: ng.IAugmentedJQuery = element.find("input");
                var labels: ng.IAugmentedJQuery = element.find('label');
                var selects: ng.IAugmentedJQuery = element.find('md-select');
                var radios: ng.IAugmentedJQuery = element.find('md-radio-group');
                var dates: ng.IAugmentedJQuery = element.find('md-datepicker');
                var datesPicker: ng.IAugmentedJQuery = element.find('mdp-date-picker');
                var textarea: ng.IAugmentedJQuery = element.find("textarea");

                var e: Object = document.querySelectorAll('div[ng-if]');
                var ifs: ng.IAugmentedJQuery = angular.element(element[0].querySelector('[ng-if]'));

                var elementos: Array<HTMLElement> = new Array<HTMLElement>();
                ...
                for (var i = 0; i < elementos.length; i++) {
                    var input: HTMLInputElement = <HTMLInputElement> elementos[i];
                    //console.debug("Input " + i);
                    if (typeof input.attributes['tipo'] != 'undefined') {
                        var tipo: string = input.attributes['tipo'].value;
                    ...
                    for (var j = 0; j < labels.length; j++) {
                        var l: HTMLLabelElement = <HTMLLabelElement>labels[j];
        ...
        static Factory(): ng.IDirectiveFactory {
            ...
            var directive = ($location: ng.ILocationService, $rootScope: ng.IScope, $compile: ng.ICompileService, $timeout: ng.ITimeoutService, $window: ng.IWindowService) => {
    ...
    export class CVDropDownDirective implements ng.IDirective {
        ...
        public template='<div id="salirFormulario">\
        ...
        constructor(private servicioCache: cvUtil.CVServicioCache,$mdpDatePicker:any,$mdMenu:any) {
            CVDropDownDirective.prototype.link = (scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes) => {}; 
        }

        public controller($scope,$mdpDatePicker,$mdMenu){
             $scope.todos="todos";
             ...
             $scope.$watch("cvActivo", function (newValue, oldValue){
                ...
                var seleccionInput:boolean=false;
...
                }
                ...
                var arrayAuxiliar=new Array<any>();
        ...
        static Factory(): ng.IDirectiveFactory {
            var directive = (servicioCache: cvUtil.CVServicioCache, $mdpDatePicker:any,$mdMenu:any) => {
    ...
    export class CVNumberFormatDirective implements ng.IDirective {
        ...
        public link: (scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes, ngModel: ng.INgModelController, filter: ng.IFilterService) => void;


        constructor(private $location: ng.ILocationService, private $rootScope: ng.IScope, private $compile: ng.ICompileService, private $filter: ng.IFilterService) {
            console.log('CVNumberFormatDirective: Inicio');
            // It's important to add `link` to the prototype or you will end up with state issues.
            // See http://blog.aaronholmes.net/writing-angularjs-directives-as-typescript-classes/#comment-2111298002 for more information.
            CVNumberFormatDirective.prototype.link = (scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes, ngModel: ng.INgModelController, filter: ng.IFilterService) => {

                console.log('CVNumberFormatDirective: Link');
                var NUMBER_FORMAT:string = '0,0.00';
                ...
                ngModel.$parsers.push((value) => {
                    ...
                    var obj2: number = null;
                    var myNumeral = numeral(value);

                    // Primero con una expresion regular
                    numeral.nullFormat('N/A');
                    var test:string=myNumeral.format(NUMBER_FORMAT);                    
                    if(test=='N/A'){
        ...
        static Factory(): ng.IDirectiveFactory {
            ...
            var directive = ($location: ng.ILocationService, $rootScope: ng.IScope, $compile: ng.ICompileService, $filter: ng.IFilterService) => {
    ...
    export class CVDateFormatDirective implements ng.IDirective {
        ...
        constructor(private $location: ng.ILocationService, private $rootScope: ng.IScope, private $compile: ng.ICompileService, private $filter: ng.IFilterService) {
            ...
            // It's important to add `link` to the prototype or you will end up with state issues.
            // See http://blog.aaronholmes.net/writing-angularjs-directives-as-typescript-classes/#comment-2111298002 for more information.
            CVDateFormatDirective.prototype.link = (scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes, ngModel: ng.INgModelController, filter: ng.IFilterService) => {
                ...
                ngModel.$parsers.push((value) => {
                    ...
                    var valido:boolean=regex.test(value);
                    if(!valido){
    ...
    export class CVPaginaUI implements ng.IDirective {
        //public $scope:ng.IScope= {};
        public templateUrl:string = '';
        //public controller:string='crearProyectoController';
        
        // @ngInject
        constructor(private servicioCache: cvUtil.CVServicioCache,t:string) { 
            this.templateUrl=cvUtil.Utilidades.getPlantilla(t,this.servicioCache);
        }

        static Factory(t:string): ng.IDirectiveFactory {
            var directive = (servicioCache: cvUtil.CVServicioCache) => {
